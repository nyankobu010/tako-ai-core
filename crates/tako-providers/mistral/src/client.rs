//! Mistral HTTP client + `LlmProvider` impl.
...
impl MistralBuilder {
    ...
    /// Mistral-specific: when true, prefix the system prompt with the
    /// platform's safety preamble.
    pub fn safe_prompt(mut self, on: bool) -> Self {
        self.safe_prompt = Some(on);
        self
    }

    /// Mistral-specific: deterministic seed for sampling.
    pub fn random_seed(mut self, seed: u64) -> Self {
        self.random_seed = Some(seed);
        self
    }

    pub fn build(self) -> Result<MistralProvider, TakoError> {
        let model = self
            .model
            .ok_or_else(|| TakoError::Invalid("MistralBuilder: model is required".into()))?;
        let api_key = self
            .api_key
            .ok_or_else(|| TakoError::Invalid("MistralBuilder: api_key is required".into()))?;
...
impl LlmProvider for MistralProvider {
    ...
    async fn chat(
        ...
        req.stream = false;
        let body =
            convert::to_mistral_request(&req, self.inner.safe_prompt, self.inner.random_seed);
        ...
        let parsed: convert::MiResponse = resp
            .json()
            .await
            .map_err(|e| TakoError::Transport(e.to_string()))?;
        convert::from_mistral_response(parsed)
    }
