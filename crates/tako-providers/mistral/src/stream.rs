//! SSE → ChatChunk adapter for Mistral's chat.completions streaming format.
//!
//! Wire format mirrors OpenAI's: `data: {...}\n\n` frames terminated by
//! `data: [DONE]`. The frame schema is OpenAI-compatible.
...
struct MiStreamFrame {
    #[serde(default)]
    choices: Vec<MiStreamChoice>,
    #[serde(default)]
    usage: Option<MiStreamUsage>,
}

#[derive(Deserialize, Debug)]
struct MiStreamChoice {
    #[serde(default)]
    delta: MiStreamDelta,
    #[serde(default)]
...
struct MiStreamDelta {
    #[serde(default)]
    content: Option<String>,
    #[serde(default)]
    tool_calls: Vec<MiStreamToolCallDelta>,
}

#[derive(Deserialize, Debug)]
struct MiStreamToolCallDelta {
    index: u32,
    #[serde(default)]
    id: Option<String>,
    #[serde(default)]
    function: Option<MiStreamFnDelta>,
}

#[derive(Deserialize, Debug)]
struct MiStreamFnDelta {
    #[serde(default)]
    name: Option<String>,
    #[serde(default)]
        ...
        while let Some(item) = events.next().await {
            match item {
                ...
                Ok(ev) => {
                    ...
                    let frame: MiStreamFrame = match serde_json::from_str(&ev.data) {
                        Ok(f) => f,
                        Err(e) => {
