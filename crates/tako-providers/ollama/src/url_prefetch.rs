...
impl Resolve for BlocklistResolver {
    fn resolve(&self, name: Name) -> Resolving {
        ...
        Box::pin(async move {
            ...
            // the host is allowlisted OR the resolved IP is in
            // an allowlisted CIDR. Per-IP check.
...
/// Phase 31.B / 32.B — operator-controlled allowlist for the URL
/// pre-fetcher. Mirror of [`tako_providers_bedrock`]'s
/// `AllowList` (per-crate copy per ARCHITECTURE.md hard rule —
/// provider crates depend only on `tako-core` + their vendor SDK
/// + reqwest; never on each other).
///
/// Three semantic forms (all bypass the private-IP blocklist
/// but NOT the scheme / timeout / size / MIME checks):
///
/// - **Exact** (Phase 30) — host string match.
/// - **Wildcard suffix** (Phase 31) — `*.X` matches any host
///   ending in `.X`, multi-level included.
/// - **CIDR subnet** (Phase 32) — IPv4 / IPv6 CIDR matches any
///   resolved IP that falls inside the network.
#[derive(Debug, Default)]
pub(crate) struct AllowList {
    exact: HashSet<String>,
    suffixes: Vec<String>,
    /// Phase 32 — IPv4 + IPv6 CIDR networks.
...
impl AllowList {
    pub(crate) fn from_strings_and_cidrs(
        ...
        for entry in cidr_entries {
            let cidr = entry.parse::<ipnet::IpNet>().map_err(|e| {
                TakoError::Invalid(format!("ollama: prefetch CIDR `{entry}` parse failed: {e}"))
            })?;
            cidrs.push(cidr);
        }
        Ok(Self {
    ...
    /// Phase 32 — true if `ip` falls inside any allowlisted CIDR.
...
pub(crate) struct UrlPrefetchOpts {
    ...
    /// [`OllamaBuilder::with_url_prefetch_allow_cidr`].
...
mod tests {
    ...
    // ----- Phase 32 — CIDR allowlist (mirror of Bedrock). -----
    ...
    /// Phase 32 end-to-end: URL `http://127.0.0.1/cat.png`
    /// allowlisted via `127.0.0.0/8` CIDR.
    #[tokio::test]
