...
1. **Bump the version**. The simplest path is a single commit on
   ...
   sed -i '' 's/version = "0\.51\.1"/version = "0.51.2"/g' Cargo.toml
   sed -i '' 's/version      = "0\.51\.1"/version      = "0.51.2"/g' Cargo.toml
   sed -i '' 's/version = "0\.51\.1"/version = "0.51.2"/g' pyproject.toml
   # Update python facade
   sed -i '' 's/__version__ = "0\.51\.1"/__version__ = "0.51.2"/' python/tako/__init__.py
   sed -i '' 's/"0\.51\.1"/"0.51.2"/g' tests/python/test_smoke.py
...
7. **Verify the release** once the workflow finishes:
   ...
   pip install --upgrade tako-ai-core==0.51.2
